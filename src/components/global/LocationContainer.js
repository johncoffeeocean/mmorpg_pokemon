...
var LocationContainer = ({ path }) => {
    ...
    if (locationContainer === null) {
      ...
      locationContainer = new Phaser.Game({
        ...
        scene: {
          ...
          create: function () {
            let locationImageObject = this.add.image(
              this.cameras.main.centerX,
              (height * this.cameras.main.width) / width / 2,
              "location-image"
            );
            locationImageObject.setDisplaySize(
              this.cameras.main.width,
              (height * this.cameras.main.width) / width
            );
            // locationContainer.colo
            durationTextObject = this.add.text(
              (this.cameras.main.centerX * 10) / 100,
              (height * this.cameras.main.width) / width + 50,
              "",
  ...
  return <div id="location-container" className="w-full h-full"></div>;
};
